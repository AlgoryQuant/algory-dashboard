...
// NOVÁ DEFINICE PROPS PRO NAVIGACI ZPĚT
interface BacktestLabProps {
  onBack?: () => void;
}

export default function BacktestLab({ onBack }: BacktestLabProps) {
  ...
  return (
    <div className="w-full flex flex-col gap-4 relative">
      ...
      {/* NOVÁ LIŠTA PRO NÁVRAT (Tlačítko Back to Terminal) */}
      {onBack && (
        <div className="w-full flex justify-start mb-2">
          <button 
            onClick={onBack}
            className="flex items-center gap-2 px-4 py-2 bg-white/[0.02] border border-white/5 hover:border-white/10 hover:bg-white/5 text-zinc-400 hover:text-white text-xs font-mono font-medium rounded-xl transition-all duration-300 group shadow-md"
          >
            <svg className="w-4 h-4 transform group-hover:-translate-x-0.5 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
            Back to Terminal
          </button>
        </div>
      )}
          ...
          <div className="bg-zinc-950/50 backdrop-blur-xl border border-white/10 rounded-[2rem] p-6 shadow-2xl relative flex flex-col h-full">
            ...
            <div className="flex flex-col mb-6 border-b border-white/5 pb-4">
              ...
              <div className="relative">
                ...
                <AnimatePresence>
                  {showTemplates && (
                    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-full left-0 w-full mt-2 bg-zinc-900 border border-white/10 rounded-xl shadow-2xl z-40 overflow-hidden flex flex-col">
                      {TEMPLATES.map((tpl) => (
                        <button key={tpl.id} onClick={() => loadTemplate(tpl.id)} className="text-left px-4 py-4 hover:bg-white/5 border-b border-white/5 transition-colors flex flex-col gap-1">
