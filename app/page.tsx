...
export default function Home() {
  ...
  const renderFavorites = () => {
    ...
    const relevantFavs = favorites.filter(ticker => {
      ...
      // V režimu Crypto Arbitrage ukazujeme pouze Arbitrážní páry
      if (marketMode === 'CRYPTO' && cryptoMode === 'arbitrage') return isArbTicker;
      if (marketMode === 'CRYPTO' && cryptoMode === 'standard') return isCryptoTicker;
  ...
  if (isArbitrageMode && activeArbData) {
  ...
  const getGlowColor = () => {
    if (isArbitrageMode && isTradeActive) return 'shadow-[0_0_60px_rgba(59,130,246,0.05)]';
  ...
  // Select appropriate news array based on market mode
  const displayNews = marketMode === 'FOREX' ? FOREX_NEWS_MOCK : CRYPTO_NEWS_MOCK;
          ...
          <div className="p-8 pb-4 border-b border-white/5 mb-4">
            ...
            {marketMode === 'CRYPTO' && (
              <div className="flex bg-[#0a0a0a] rounded-full p-1 mt-3 border border-white/5 shadow-inner">
                ...
                <button 
                  onClick={() => { setCryptoMode('arbitrage'); setActivePair("BTCUSD/ETHUSD"); }} 
          ...
          <nav className="flex-1 overflow-y-auto pb-10 custom-scrollbar pr-2 pl-2 flex flex-col">
            ...
            <div className="pb-20 flex-1">
              {marketMode === 'FOREX' ? (
                ...
                <>{renderSidebarGroup('Pairs Trading (Stat Arb)', data.crypto_arb, true)}</>
              ) : (
            ...
            {loading && !data.majors ? (
              ...
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-10 mt-10">
                <div className="xl:col-span-2 flex flex-col space-y-10">
                  <TradingChart symbol={activePair} isArb={isArbitrageMode} />
                  
                  <div className={`bg-[#0a0a0a]/80 backdrop-blur-2xl border ${isArbitrageMode && isTradeActive ? 'border-blue-500/20' : inferredDirection === 'SELL' ? 'border-red-500/20' : inferredDirection === 'BUY' ? (marketMode === 'CRYPTO' ? 'border-blue-500/20' : 'border-emerald-500/20') : 'border-white/5'} rounded-[2rem] overflow-hidden p-8 transition-all duration-700 ${getGlowColor()}`}>
                    
                    {isArbitrageMode && activeArbData ? (
                      /* --- ARBITRAGE CARD CONTENT --- */
                      <>
                          ...
                          <div className="w-full">
                            <div className="flex items-center gap-4 mb-4">
                              ...
                              <span className="px-3 py-1 bg-blue-500/10 text-blue-400 text-[10px] uppercase tracking-widest rounded-lg border border-blue-500/30 font-bold flex items-center">
                                ...
                                <InfoTooltip term="Statistical Arbitrage" info="A quantitative strategy capitalizing on the temporary divergence of historically cointegrated assets. Trades mean reversion." />
                              </span>
                            </div>
                      ...
                      /* --- STANDARD (DIRECTIONAL) CARD CONTENT --- */
                      <>
