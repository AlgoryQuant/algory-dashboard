...
interface DashboardData {
  ...
  crypto_arb?: Record<string, StatArbData>;
  news?: NewsItem[];
...
const InfoTooltip = ({ term, info }: { term: string, info: string }) => (
  <span className="relative group inline-flex items-center cursor-help ml-2">
    <span className="flex items-center justify-center w-3.5 h-3.5 text-[9px] border border-zinc-600 text-zinc-400 rounded-full hover:bg-white/10 hover:text-white transition-colors">i</span>
...
const PositionCalculator = ({ slPips, direction }: { slPips: number, direction: string }) => {
  ...
  return (
    <div className="mt-6 p-6 bg-black/30 rounded-2xl border border-white/5 shadow-inner">
      <div className="text-xs font-bold text-zinc-400 uppercase tracking-widest mb-4 flex items-center">Position Sizing</div>
      <div className="flex gap-6 items-end">
        <div className="flex flex-col gap-2 w-1/3">
          <label className="text-[10px] text-zinc-500 uppercase font-semibold">Balance ($)</label>
          <input type="number" value={balance} onChange={(e) => setBalance(Number(e.target.value))} className={`bg-zinc-900/80 border border-white/5 rounded-lg px-4 py-2 text-sm text-white focus:outline-none focus:ring-1 ${focusRingColor} transition-all`} />
        </div>
        <div className="flex flex-col gap-2 w-1/3">
          <label className="text-[10px] text-zinc-500 uppercase font-semibold">Risk (%)</label>
          <input type="number" step="0.1" value={riskPercent} onChange={(e) => setRiskPercent(Number(e.target.value))} className={`bg-zinc-900/80 border border-white/5 rounded-lg px-4 py-2 text-sm text-white focus:outline-none focus:ring-1 ${focusRingColor} transition-all`} />
        </div>
        <div className={`w-1/3 flex flex-col items-center justify-center py-2 px-4 rounded-lg border shadow-inner ${direction === 'BUY' ? 'bg-emerald-500/5 border-emerald-500/10' : 'bg-red-500/5 border-red-500/10'}`}>
          <span className={`text-[10px] uppercase font-bold mb-1 ${direction === 'BUY' ? 'text-emerald-500/70' : 'text-red-500/70'}`}>Volume</span>
...
const MarketMonitor = ({ lastRefresh, mode }: { lastRefresh: Date | null, mode: string }) => {
  ...
  return (
    <div className="mb-10 p-8 bg-white/[0.02] backdrop-blur-3xl border border-white/5 rounded-[2rem] shadow-2xl relative overflow-hidden">
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-8 relative z-10">
        <div className="flex flex-col gap-2">
          <div className="text-5xl font-semibold tracking-tight text-white">{now.toLocaleTimeString('en-US', { hour12: false, hour: '2-digit', minute: '2-digit' })}<span className="text-2xl text-white/50 ml-1">:{now.getSeconds().toString().padStart(2, '0')}</span></div>
          <div className="text-xs font-medium text-zinc-400 uppercase tracking-widest flex items-center gap-3 mt-2">
            <span className="flex items-center gap-2"><span className={`w-1.5 h-1.5 rounded-full animate-pulse ${isCrypto ? 'bg-blue-400' : 'bg-emerald-400'}`}></span>System Sync ({mode})</span>
        ...
        <div className="flex flex-wrap gap-3">
          {sessions.map((s) => (
            <div key={s.name} className={`px-5 py-3 border rounded-xl flex flex-col items-center justify-center ${s.isActive ? `${isCrypto ? 'bg-blue-500/10 border-blue-500/30 shadow-[0_0_15px_rgba(59,130,246,0.1)]' : 'bg-emerald-500/10 border-emerald-500/30 shadow-[0_0_15px_rgba(52,211,153,0.1)]'}` : 'bg-black/40 border-white/5 opacity-60'}`}>
              <span className={`text-xs font-bold uppercase tracking-widest mb-1 ${s.isActive ? (isCrypto ? 'text-blue-400' : 'text-emerald-400') : 'text-zinc-500'}`}>{s.name}</span>
      ...
      <div className="mt-8 relative z-10">
        <div className="flex justify-between text-[10px] text-zinc-500 font-bold mb-3 uppercase tracking-widest"><span>AI Engine M15 Cycle</span><span>{15 - (minutes % 15)}m {(60 - seconds) % 60}s remaining</span></div>
        <div className="w-full h-2 bg-black/60 rounded-full overflow-hidden border border-white/5"><div className={`h-full rounded-full transition-all duration-1000 ease-linear ${isCrypto ? 'bg-gradient-to-r from-blue-600 to-blue-400' : 'bg-gradient-to-r from-emerald-600 to-emerald-400'}`} style={{ width: `${progressPercent}%` }} /></div>
      </div>
    </div>
  );
};

// === SIDEBAR ITEM KOMPONENTA ===
...
const SidebarItemNode = ({ ticker, prob, arbData, isActive, isFavorite, onClick, onToggleFavorite, isOverlay, dragListeners, dragAttributes, setNodeRef, style }: SidebarItemProps) => {
  ...
  } else if (isArb && arbData) {
      if (arbData.ZScore > 3.5 || arbData.ZScore < -3.5) probColor = isActive ? "text-blue-400" : "text-blue-500/80";
  }
  ...
  if (isOverlay) containerClasses += `bg-[#0a0a0a] border-white/20 shadow-2xl ring-1 ring-white/10 scale-105 rotate-2 z-50`;
    ...
    if (isArb) containerClasses += 'bg-blue-500/10 border-blue-500/20 shadow-[0_0_15px_rgba(59,130,246,0.05)] ';
    else containerClasses += pairDir === 'SELL' ? 'bg-red-500/10 border-red-500/20 shadow-[0_0_15px_rgba(239,68,68,0.05)] ' : 'bg-emerald-500/10 border-emerald-500/20 shadow-[0_0_15px_rgba(52,211,153,0.05)] ';
    ...
    <div ref={setNodeRef} style={style} className={containerClasses} onClick={!isOverlay ? onClick : undefined}>
      ...
      <div className="flex items-center gap-3">
        <span className={`text-[10px] font-bold tracking-widest ${probColor}`}>
          {isArb ? `Z: ${arbData?.ZScore.toFixed(1) ?? '0.0'}` : `${((prob ?? 0) * 100).toFixed(0)}%`}
        </span>
        <button onClick={(e) => { e.stopPropagation(); onToggleFavorite(ticker); }} className={`transition-all duration-300 hover:scale-110 ${isFavorite ? 'text-zinc-300 hover:text-red-400' : 'text-zinc-600 hover:text-white'}`} title={isFavorite ? "Remove" : "Add"}>
...
export default function Home() {
  ...
  const [openGroups, setOpenGroups] = useState<Record<string, boolean>>({
    'Major Liquidity': true, 'Cross Pairs': true, 'Precious Metals': true, 'Crypto Assets': true, 'Pairs Trading (Stat Arb)': true
  });
  ...
  const getSidebarIcon = (title: string) => {
    ...
    if (title.includes('Stat Arb')) return <svg className="w-3.5 h-3.5 text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2"><path d="M2 12h4l3-9 5 18 3-9h5" /></svg>;
    return null;
  };

  const renderSidebarGroup = (title: string, pairs: Record<string, any> | undefined, isArb: boolean = false) => {
    if (!pairs || Object.keys(pairs).length === 0) return null;
    const availablePairs = Object.entries(pairs).filter(([ticker]) => !favorites.includes(ticker));
    if (isArb) availablePairs.sort((a, b) => Math.abs(b[1].ZScore) - Math.abs(a[1].ZScore));
    else availablePairs.sort((a, b) => b[1] - a[1]);
    const isOpen = openGroups[title]; 

    return (
      <div className="mb-6">
        <button onClick={() => setOpenGroups(prev => ({ ...prev, [title]: !prev[title] }))} className="w-full flex items-center justify-between px-6 py-2 mb-3 cursor-pointer group outline-none">
          <div className="flex items-center gap-2">
            {getSidebarIcon(title)}
            <span className={`text-[10px] font-bold uppercase tracking-widest transition-colors ${isArb ? 'text-blue-500/80 group-hover:text-blue-400' : 'text-zinc-500 group-hover:text-zinc-300'}`}>{title}</span>
          </div>
          <svg className={`w-3.5 h-3.5 text-zinc-600 group-hover:text-zinc-400 transition-transform duration-300 ${isOpen ? 'rotate-180' : 'rotate-0'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
        </button>
        <div className={`space-y-1.5 px-3 overflow-hidden transition-all duration-500 ease-in-out ${isOpen ? 'max-h-[1500px] opacity-100' : 'max-h-0 opacity-0'}`}>
          {availablePairs.map(([ticker, val]) => (
            <SidebarItemNode key={ticker} ticker={ticker} prob={isArb ? undefined : val} arbData={isArb ? val : undefined} isActive={activePair === ticker} isFavorite={false} onClick={() => setActivePair(ticker)} onToggleFavorite={toggleFavorite} />
          ))}
        </div>
      </div>
    );
  ...
  const renderFavorites = () => {
    ...
    const relevantFavs = favorites.filter(ticker => {
      ...
      const isArbTicker = Object.keys(data.crypto_arb || {}).includes(ticker);
      if (marketMode === 'CRYPTO') return isCryptoTicker || isArbTicker;
      if (marketMode === 'FOREX') return !isCryptoTicker && !isArbTicker;
      return true;
    });
    ...
    return relevantFavs.map(ticker => {
      const isArb = Object.keys(data.crypto_arb || {}).includes(ticker);
      return <SortableSidebarItem key={ticker} ticker={ticker} prob={isArb ? undefined : allPairsMap[ticker] || 0} arbData={isArb ? data.crypto_arb![ticker] : undefined} isActive={activePair === ticker} isFavorite={true} onClick={() => setActivePair(ticker)} onToggleFavorite={toggleFavorite} />;
    });
  };
  ...
  const activeArbData = data.crypto_arb?.[activePair];
  const isArb = !!activeArbData;
  const displayTicker = activePair === "XAUUSD" ? "GOLD (XAUUSD)" : activePair;

  let inferredDirection = "NEUTRAL";
  let isTradeActive = false;
  let gaugeRotation = 0;
  let needleColor = '#a1a1aa';

  if (isArb && activeArbData) {
    const clampedZ = Math.max(-5, Math.min(5, activeArbData.ZScore));
    gaugeRotation = (clampedZ / 5) * 90;
    needleColor = activeArbData.ZScore > 3.5 ? '#ef4444' : (activeArbData.ZScore < -3.5 ? '#34d399' : '#a1a1aa');
    isTradeActive = activeArbData.Signal !== "WAIT (Z-Score is neutral)";
  } else {
    const clampedProb = Math.max(0, Math.min(1, activeProb));
    gaugeRotation = (clampedProb * 180) - 90; 
    ...
    needleColor = inferredDirection === 'BUY' ? '#34d399' : inferredDirection === 'SELL' ? '#f87171' : '#a1a1aa';
  }

  const getPageBackground = () => {
    if (isArb) return 'from-blue-950/20 via-[#0a0a0a] to-[#050505]';
  ...
  const getGlowColor = () => {
    if (isArb && isTradeActive) return 'shadow-[0_0_60px_rgba(59,130,246,0.05)]';
      ...
      <div className="flex h-screen bg-[#050505] text-zinc-200 selection:bg-emerald-500/30 overflow-hidden font-sans animate-in fade-in duration-700">
        ...
        <aside className="w-80 flex-shrink-0 border-r border-white/10 bg-[#050505] flex flex-col h-full z-20 hidden md:flex">
          <div className="p-8 pb-6 border-b border-white/5 mb-4">
            ...
            <div className="flex bg-black/60 rounded-xl p-1 mt-6 border border-white/5">
              ...
              <button onClick={() => { setMarketMode('CRYPTO'); setActivePair("BTCUSD"); }} className={`flex-1 text-[10px] font-bold tracking-widest uppercase py-2 rounded-lg transition-all ${marketMode === 'CRYPTO' ? 'bg-white/10 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}>Crypto</button>
            </div>
          </div>
          ...
          <nav className="flex-1 overflow-y-auto pb-10 custom-scrollbar pr-2 pl-2 flex flex-col">
            ...
            <div className="pb-20 flex-1">
              {marketMode === 'FOREX' ? (
                ...
                <>{renderSidebarGroup('Crypto Assets', data.crypto)}{renderSidebarGroup('Pairs Trading (Stat Arb)', data.crypto_arb, true)}</>
              )}
            </div>
          </nav>
        </aside>
        ...
        <main className={`flex-1 overflow-y-auto custom-scrollbar px-6 pt-12 pb-24 lg:px-12 lg:pt-20 scroll-smooth transition-colors duration-1000 ease-in-out bg-gradient-to-br ${getPageBackground()}`}>
          <div className="max-w-[1400px] mx-auto">
            <MarketMonitor lastRefresh={lastRefresh} mode={marketMode as string} />

            {loading && !data.majors ? (
              <div className="p-20 mt-10 text-center flex flex-col items-center justify-center gap-6 border border-white/10 rounded-[2rem] bg-white/[0.02]"><div className={`w-10 h-10 border-4 border-t-transparent rounded-full animate-spin ${marketMode === 'CRYPTO' ? 'border-blue-500/30 border-t-blue-500' : 'border-emerald-500/30 border-t-emerald-500'}`}></div><span className="text-sm text-zinc-400 font-medium tracking-widest uppercase">Connecting to Engine...</span></div>
            ) : error && !data.majors ? (
              ...
              <div className="grid grid-cols-1 xl:grid-cols-3 gap-10 mt-10">
                <div className="xl:col-span-2 flex flex-col space-y-10">
                  <TradingChart symbol={activePair} isArb={isArb} />
                  
                  <div className={`bg-[#0a0a0a]/80 backdrop-blur-2xl border ${isArb && isTradeActive ? 'border-blue-500/20' : inferredDirection === 'SELL' ? 'border-red-500/20' : inferredDirection === 'BUY' ? (marketMode === 'CRYPTO' ? 'border-blue-500/20' : 'border-emerald-500/20') : 'border-white/5'} rounded-[2rem] overflow-hidden p-8 transition-all duration-700 ${getGlowColor()}`}>
                    
                    {isArb ? (
                      /* --- ARBITRÁŽNÍ OBSAH KARTY --- */
                      <>
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 border-b border-white/5 pb-8">
                          <div className="w-full">
                            <div className="flex items-center gap-4 mb-4">
                              <h2 className="text-4xl font-bold text-white tracking-tight">{activePair.replace('/', ' / ')}</h2>
                              <span className="px-3 py-1 bg-blue-500/10 text-blue-400 text-[10px] uppercase tracking-widest rounded-lg border border-blue-500/30 font-bold flex items-center">
                                Statistical Arbitrage
                                <InfoTooltip term="Statistical Arbitrage" info="Kvantitativní strategie těžící z dočasného vychýlení historicky kointegrovaných aktiv. Obchoduje se návrat k průměru (Mean Reversion)." />
                              </span>
                            </div>
                            <div className="flex flex-wrap items-center gap-4 mt-6">
                              <div className="flex flex-col px-5 py-3 bg-black/40 rounded-xl border border-white/5 shadow-inner">
                                <span className="text-zinc-500 text-[10px] uppercase tracking-widest mb-1">Current Spread</span>
                                <span className="text-white font-mono text-lg font-bold">{activeArbData?.Spread.toFixed(4)}</span>
                              </div>
                              <div className="flex flex-col px-5 py-3 bg-black/40 rounded-xl border border-white/5 shadow-inner">
                                <span className="text-zinc-500 text-[10px] uppercase tracking-widest mb-1">Historical Mean</span>
                                <span className="text-white/80 font-mono text-lg">{activeArbData?.Mean.toFixed(4)}</span>
                              </div>
                              <div className="flex flex-col px-5 py-3 bg-black/40 rounded-xl border border-white/5 shadow-inner">
                                <span className="text-zinc-500 text-[10px] uppercase tracking-widest mb-1">Standard Deviation</span>
                                <span className="text-white/80 font-mono text-lg">{activeArbData?.StdDev.toFixed(4)}</span>
                              </div>
                            </div>
                          </div>
                          <div className="flex flex-col items-center gap-6 flex-shrink-0">
                            <div className="flex flex-col items-center justify-center relative w-64 h-32 mt-2">
                              <svg viewBox="0 0 200 120" className="w-full h-full drop-shadow-2xl overflow-visible">
                                <path d="M 30 100 A 70 70 0 0 1 70 40" fill="none" stroke="#34d399" strokeWidth="6" strokeLinecap="round" strokeOpacity="0.8" />
                                <path d="M 70 40 A 70 70 0 0 1 130 40" fill="none" stroke="#a1a1aa" strokeWidth="6" strokeLinecap="round" strokeOpacity="0.3" />
                                <path d="M 130 40 A 70 70 0 0 1 170 100" fill="none" stroke="#ef4444" strokeWidth="6" strokeLinecap="round" strokeOpacity="0.8" />
                                <text x="25" y="115" fontSize="8" fill="#34d399" fontWeight="bold" textAnchor="middle">LONG A</text>
                                <text x="175" y="115" fontSize="8" fill="#ef4444" fontWeight="bold" textAnchor="middle">SHORT A</text>
                              ...
                              <div className="absolute bottom-[-20px] flex flex-col items-center">
                                <span className="text-[10px] text-zinc-500 uppercase tracking-widest font-bold">Z-Score</span>
                                <div className={`text-4xl font-black tracking-tighter ${activeArbData!.ZScore > 3.5 ? 'text-red-400' : (activeArbData!.ZScore < -3.5 ? 'text-emerald-400' : 'text-white')}`}>
                                  {activeArbData!.ZScore > 0 ? '+' : ''}{activeArbData?.ZScore.toFixed(2)}
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="mt-8 flex justify-between items-center bg-black/40 p-6 rounded-2xl border border-white/5">
                          <div className="flex flex-col">
                             <span className="text-zinc-500 text-[10px] uppercase tracking-widest mb-1">Algorithm Signal</span>
                             <span className={`text-xl font-bold ${isTradeActive ? 'text-blue-400' : 'text-zinc-400'}`}>{activeArbData?.Signal}</span>
                          </div>
                          {isTradeActive ? (
                             <button className="px-8 py-4 bg-blue-500 text-white font-bold text-xs tracking-widest uppercase rounded-xl transition-all hover:bg-blue-400 hover:-translate-y-1 shadow-[0_5px_20px_rgba(59,130,246,0.2)]">Execute Arbitrage</button>
                          ) : (
                             <button disabled className="px-8 py-4 bg-zinc-900/50 text-zinc-600 font-bold text-xs tracking-widest uppercase rounded-xl border border-white/5 cursor-not-allowed">Mean Reversion Inactive</button>
                          )}
                        </div>
                      </>
                    ) : (
                      /* --- SMĚROVÝ (NORMAL) OBSAH KARTY --- */
                      <>
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8 border-b border-white/5 pb-8">
                          <div className="w-full">
                            <div className="flex items-center gap-4 mb-4">
                              ...
                              {isTradeActive && (
                                <span className={`px-4 py-1.5 text-[11px] font-bold uppercase tracking-widest rounded-lg border shadow-lg ${inferredDirection === 'BUY' ? (marketMode === 'CRYPTO' ? 'bg-blue-500/10 text-blue-400 border-blue-500/30' : 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30') : 'bg-red-500/10 text-red-400 border-red-500/30'}`}>{inferredDirection} PENDING</span>
                              )}
                            ...
                            {activeParams && (
                              <div className="flex flex-wrap items-center gap-3 mt-4">
                                ...
                                <span className="px-3 py-1.5 bg-black/40 rounded-lg border border-white/5 font-mono text-[11px]"><span className="text-zinc-500 mr-2 uppercase">TP</span><span className="text-white font-bold">{activeParams.TP === 9999 ? 'OPEN' : activeParams.TP}</span></span>
                                {activeParams.RRR && <span className="px-3 py-1.5 bg-zinc-800/50 rounded-lg border border-zinc-700/50 font-mono text-[11px]"><span className="text-zinc-400 mr-2 uppercase font-bold">RRR</span><span className="text-white font-bold">1:{activeParams.RRR}</span></span>}
                              </div>
                            )}
