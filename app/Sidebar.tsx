  ...
  return (
    <>
      ...
      <aside className="w-80 flex-shrink-0 border-r border-white/10 bg-zinc-950/50 backdrop-blur-xl flex flex-col h-full z-20 hidden lg:flex overflow-hidden shadow-2xl">
        <div className="p-8 pb-4 border-b border-white/5 mb-4 flex-shrink-0">
          ...
          <div className="flex bg-zinc-900/80 rounded-xl p-1 mt-6 border border-white/5 shadow-inner relative overflow-hidden">
            {!isParentAccount && (
              <>
                <button 
                  ...
                  className={`flex-1 z-10 text-[9px] font-bold tracking-widest uppercase py-2 rounded-lg transition-all duration-300 ${activeView === 'terminal' ? 'text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
                >
                  TERMINAL
                </button>
                <button 
                  onClick={() => { SoundEngine.playClick(); setActiveView('laboratory'); }} 
                  className={`flex-1 z-10 text-[9px] font-bold tracking-widest uppercase py-2 rounded-lg transition-all duration-300 flex items-center justify-center gap-1 ${activeView === 'laboratory' ? 'text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
                >
            ...
            <button 
              ...
              className={`flex-1 z-10 text-[9px] font-bold tracking-widest uppercase py-2 rounded-lg transition-all duration-300 ${activeView === 'portfolio' ? 'text-white' : 'text-zinc-500 hover:text-zinc-300'}`}
            >
              PORTFOLIO
            </button>
          ...
          {activeView === 'terminal' && !isParentAccount && (
        ...
        {/* ASYMETRICKÉ VYTVOŘENÍ SIDEBAR MENU PODLE ROLE */}
        {isParentAccount ? (
          <div className="flex-1 flex flex-col items-center justify-center p-6 opacity-50">
            <svg className="w-8 h-8 text-zinc-500 mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {/* ─── MOBILE BOTTOM NAVIGATION (Ukotveno na mobilech dole, z-index 100) ─── */}
      <div className="lg:hidden fixed inset-x-0 bottom-0 bg-zinc-950/95 backdrop-blur-2xl border-t border-white/10 z-[100] flex flex-col shadow-[0_-10px_40px_rgba(0,0,0,0.8)] font-sans pb-safe">
        
        {activeView === 'terminal' && marketMode && !isParentAccount && (
