...
const StatusBadge = ({ status }: { status: ArbStatus }) => {
  const colors = { ACTIVE: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20', DEGRADING: 'text-yellow-400 bg-yellow-500/10 border-yellow-500/20', CLOSED: 'text-red-400 bg-red-500/10 border-red-500/20' };
  const dotColors = { ACTIVE: 'bg-emerald-400', DEGRADING: 'bg-yellow-400', CLOSED: 'bg-red-400' };
  return (
      <div className={`px-2 py-1 rounded-md border text-[9px] font-bold tracking-widest flex items-center gap-1.5 ml-4 ${colors[status]}`}>
          <span className={`w-1.5 h-1.5 rounded-full ${dotColors[status]} ${status === 'ACTIVE' ? 'animate-pulse' : ''}`}></span>{status}
      </div>
  )
};
      ...
      <div className="h-48 w-full">
          <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data} margin={{ top: 5, right: 0, left: -20, bottom: 0 }}>
                  <defs><linearGradient id={`colorGradient-${color}`} x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor={color} stopOpacity={0.3}/><stop offset="95%" stopColor={color} stopOpacity={0}/></linearGradient></defs>
                  ...
                  <RechartsTooltip contentStyle={{ backgroundColor: '#0a0a0a', borderColor: 'rgba(255,255,255,0.1)', borderRadius: '0.5rem', fontSize: '12px' }} itemStyle={{ color: '#fff', fontWeight: 'bold' }} labelStyle={{ color: '#a1a1aa', marginBottom: '4px' }} />
      ...
      <div className="p-8">
        ...
        <div className="bg-[#0a0a0a] border border-white/10 rounded-2xl p-8">
          <div className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest mb-6 flex items-center gap-2">
            <svg className="w-4 h-4 text-blue-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>NET PROFIT CALCULATOR
          </div>
          ...
          <div className="mt-8 flex justify-end"><ExecuteButton baseClass="w-full md:w-auto px-10 py-5 font-bold text-[10px] tracking-widest uppercase rounded-xl" defaultText="EXECUTE ARBITRAGE" colorTheme="blue" disabled={arbData.status === 'CLOSED'} /></div>
        </div>
    ...
    <div className="w-full bg-[#050505]/80 backdrop-blur-2xl border border-orange-500/20 rounded-[2rem] overflow-hidden shadow-[0_0_50px_rgba(249,115,22,0.15)] transition-all duration-300 relative z-10">
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
          {[ { id: 'ex-binance', name: 'Binance', rate: data.binanceRate }, { id: 'ex-bybit', name: 'Bybit', rate: data.bybitRate }, { id: 'ex-okx', name: 'OKX', rate: data.okxRate } ].map(ex => (
        ...
        <div className="bg-orange-950/20 border border-orange-500/20 rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex gap-4 items-center">
            <div className="px-6 py-4 bg-emerald-500/10 border border-emerald-500/30 rounded-xl flex flex-col items-center">
              <span className="text-[10px] text-emerald-500 uppercase font-bold tracking-widest mb-1">LONG POSITION</span><span className="text-xl font-bold text-white uppercase">{data.optimalLong}</span>
            </div>
            ...
            <div className="px-6 py-4 bg-red-500/10 border border-red-500/30 rounded-xl flex flex-col items-center">
              <span className="text-[10px] text-red-500 uppercase font-bold tracking-widest mb-1">SHORT POSITION</span><span className="text-xl font-bold text-white uppercase">{data.optimalShort}</span>
            </div>
          </div>
